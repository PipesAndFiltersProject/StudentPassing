...
bool threading = true; // If true, save the data files in separate threads to boost performance.
int studentCount = 0;
int exerciseCount = 6;
int bufSize = 100;
...
int main(int argc, char ** argv) {
   ...
   while (!generatedStudentNumbers.empty()) {
      ...
      if (bufferCounter >= bufSize) {
         if (threading) {
            launchSaveThreadsAndWait();
         } else { // serialized
            saveSerialized(isFirstRound);
            isFirstRound = false;
         }
         bufferCounter = 0;
         basicInfoBuffer.resize(bufSize);
...
int parseArgs(int argc, char ** argv) {
   Sarge sarge;
   sarge.setUsage("./GenerateTestData -[hvt]s <number> [-e <number>] -b <number>");
   ...
   sarge.setArgument("z", "serialized", "Save the data files sequentially, not using threads.", false);
   ...
   // and at the end.
   bufSize = std::min(studentCount, 100);
